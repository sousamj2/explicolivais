...
def request_confirmation():
    ...
    if request.method == "POST":
        ...
        status = insertNewRegistrationToken(token, ip_addr, email)
        if status != "Insert successful":
             flash("Ocorreu um erro ao processar o seu pedido. Por favor, tente novamente mais tarde.")
             print(f"Error inserting registration token: {status}", flush=True)
             return redirect(url_for("register.request_confirmation"))
